...
def get_valid_neighbour(m, row, col):
    val = m[row][col]
    total = 0
    if val == 9:
        m[row][col] = None
    ...
    for x, y in [(0, 1), (1, 0), (0, -1), (-1, 0)]:
        ...
        if m[new_row][new_col] == val + 1:
            total += get_valid_neighbour(m, new_row, new_col)
    return total
...
print(sum(get_valid_neighbour(copy.deepcopy(m), *trailhead) for trailhead in trailheads))
