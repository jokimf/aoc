...
def determine_hand_type(hand:str):
    ...
    if 'J' not in hand:
       ...
       # Add card that occurs most again
        distribution = Counter(hand)
        most = distribution.most_common()[0][0]

        if most == 'J':
            if distribution.most_common()[0][1] == 5:
                return 7
            second_most = distribution.most_common()[1][0]
            hand = hand.replace('J', second_most)
        else:
